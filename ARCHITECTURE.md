...
stateDiagram-v2
    [*] --> guard
    guard --> [*]: not a use case /\ninstruction payload
    guard --> plan_queries
    plan_queries --> retrieve
    retrieve --> propose
    propose --> critique
    critique --> propose: objection\n(first time only)
    critique --> gate: sustain, or\nrevision spent
    gate --> report: human approves\nor overrides
    report --> [*]
```
...
| `plan_queries` (Flash) | 2–4 queries in statute vocabulary | Paraphrase gap: user language misses Act terms | Measured, not assumed: BM25-only on raw descriptions = 25.0% recall@5 in the committed scorecard; transformation + dense leg are the answer, and the scorecard compares configs |
